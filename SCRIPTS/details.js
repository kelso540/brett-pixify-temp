const meals = [
 ...
 let currentId;
  ...
  window.onload = function () {
    //grab all parameters from url
    const urlParams = new URLSearchParams(window.location.search);
    currentId = urlParams.get("id");
    
    //get the one meal that matches the id from the user input





    // NOTES: added == here instead of = you want == instead of === so we ignore the type(string, boolean, number).
    const currentMeal = meals.filter(item => item.id == currentId);
    ...
    currentCategory = currentMeal[0].category;
    const meal = `<div class="current-meal">
      <div class="current-meal-info">
        <h2>${currentMeal[0].name}</h2>
        <p class="current-meal-category">${currentMeal[0].category}</p>
      </div>
      <img src=${currentMeal[0].imageUrl} alt=${currentMeal[0].name} class="current-meal-image" />
    </div>`
 const mealContainer = document.querySelector("main");
 mealContainer.innerHTML = meal;
  }
...
//more in category
const moreMealsBtn = document.querySelector("#more-meals") 
moreMealsBtn.addEventListener('click',()=>{
  // console.log(currentCategory)
  const similarMeals = meals
  .filter(item=>item.category===currentCategory && item.id!=currentId)
  .map(item=>{
  ...
  const similarMealsContainer = document.createElement("div")
  similarMealsContainer.setAttribute("id","similar-meals-container")
  similarMealsContainer.innerHTML = similarMeals;

  const mealContainer = document.querySelector("main")
  mealContainer.appendChild(similarMealsContainer)

  moreMealsBtn.disabled = true;
})